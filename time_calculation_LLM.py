...
# from LLM_util import process_gemm_shapes, linear_gemm, getTotMemReq
...
class TimeCalculationLLM(TimeCalculation):
    ...
    def calcTime_LLM(self):
        ...
        # reduction for embedding layer, reduction occurs between data parallel (dp) GPUs of different nodes, all-reduce
        # (total number of GPUs for all-reduce equals dp)
        R_embedding = self.getR(
            Dim0=vocab_size, Dim1=hidden_dim, p=self.dp, ib=self.IBD, ll=self.LLD,
            partial=False, allReduce=True, name="embedding reduction"
        )
        R_embedding += self.getR(
            Dim0=seq_len, Dim1=hidden_dim, p=self.dp, ib=self.IBD, ll=self.LLD,
            partial=False, allReduce=True, name="positional encoding reduction"
        )
        #reduction for linear softmax layer, occurs between dp GPUs of different nodes, all-reduce
        # (total number of GPUs for all-reduce equals dp)
        R_linear_softmax = self.getR(
            Dim0=hidden_dim, Dim1=vocab_size, p=self.dp, ib=self.IBD, ll=self.LLD,
            partial=False, allReduce=True, name="softmax reduction"
        )

        # Print reduction times if debugging
        if debug:
            ...
            print(f"Data Parallel Transformer Reduction Time: {R_transformer * m:.1f}{second}")

        # Calculate inter-layer communication latency
        Tf = self.getInterLayerCommLatency_LLM(batch_size, hidden_dim, seq_len) if self.lp > 1 else 0
        Tb = Tf
        ...
        g = simulate_LLM.Graph(
            ...
            Tb=Tb, Tf=Tf, T_reduction_transformer=R_transformer, T_grad_transformer=G_transformer,
            T_reduction_embedding=R_embedding, T_reduction_linear_softmax=R_linear_softmax,
            T_transformer_f=transformer_time_f, T_transformer_b=transformer_time_b
        )

        time_fw, time_bw, total_time = g.save_graph()
        ...
        with open(output_file, "w") as f:
            ...
            f.write("Total Time: {0:.8f} {1}\n".format(total_time * m, second))
