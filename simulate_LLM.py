...
class Edge:
  def __init__(self, name, op_id, link_id, duration, is_all_reduce=False):
    self.name = name
    self.op_id = op_id
...
######################backward
        ...
        for b in reversed(range(self.num_batch)): #connect each data batch node with corresponding nodes
            ...
            R_edge[b].append(Edge("Reduce_Embedding", op_id, self.lp , self.T_reduction_embedding, is_all_reduce=True))
            op_id += 1
            if self.all_reduce == "the end":
                R_edge[b].append(Edge("Reduce_transformer", op_id, self.lp  , self.T_reduction_transformer, is_all_reduce=True))
                op_id += 1

                R_edge[b].append(Edge("Reduce_Softmax", op_id,  self.lp, self.T_reduction_linear_softmax, is_all_reduce=True))
                op_id += 1
    ...
    def save_graph(self, output_folder = "output_graph/"):
        ...
        time_fw = self.simulate(fw_roots)
        if not os.path.exists(output_folder):
            os.makedirs(output_folder)

        filename = "fwd_graph_num_batch%s_layer%s_lp%s" % (self.num_batch, self.num_layer, self.lp)
        filename_bwd = "bwd_graph_num_batch%s_layer%s_lp%s" % (self.num_batch, self.num_layer, self.lp)
        filename_fw_bw = "fw_bw_graph_num_batch%s_layer%s_lp%s" % (self.num_batch, self.num_layer, self.lp)
        
        dot_fw = visualize_graph(fw_roots, filename=filename)
        ...
        time_bw = self.simulate(bw_roots)   
        
        dot_bw = visualize_graph(bw_roots, filename=filename + "_bwd")
        ...
        fw_bw_roots = self.construct_fwd_bwd_graph()
        time = self.simulate(fw_bw_roots[0])

        dot_fw_bw = visualize_graph(fw_bw_roots[0], filename=filename_fw_bw)
        dot_fw_bw.render(output_folder + filename_fw_bw, format="png", cleanup=True)
        print("Forward-Backward graph saved to %s%s.png" % (output_folder, filename_fw_bw))
        print("Forward-Backward simulation time: {}".format(time))

        return time_fw, time_bw , time
